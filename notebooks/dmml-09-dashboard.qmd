...
format:
    dashboard: default
theme: flatly # There are several other themes available, see https://quarto.org/docs/dashboards/theming.html#overview or https://bootswatch.com/
---
...
If you want to restrict access to your dashboard, you can place a `.htaccess` file in the same directory as your dashboard file with the following content:

```
require niifEduPersonHeldCourse abgtm22ga
```

This ensures that only the staff members of this course can access the contents of that directory.

You can read more about this in the documentation section on the [Caesar Infopanel](https://info.caesar.elte.hu/){target="_blank"} under the third topic.

Unfortunately, the user directive listed in the documentation does not seem to work (i.e, specifying users with `require user user1@student.elte.hu user2@student.elte.hu (...)`), so even you will not be able to access your own dashboard if you try to use that. As a result, you will not be able to access your own dashboard if you try to use it. Therefore, during the presentation session, you may need to showcase your dashboard from your local machine or temporarily remove the `.htaccess` file.
...
Source: [Liver Cirrhosis Prediction 💊 | EDA + Models (Kaggle).](https://www.kaggle.com/code/vizeno/liver-cirrhosis-prediction-eda-models)
